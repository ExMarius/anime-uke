...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const weekly = await env.DB
      .prepare(
        `SELECT u.username, u.level, u.rank_theme, u.is_admin, u.staff_role, up.avatar_url AS avatar,
    ...
    const alltime = await env.DB
      .prepare(
        `SELECT u.username, u.points, u.level, u.rank_theme, u.is_admin, u.staff_role, up.avatar_url AS avatar
         FROM users u
         LEFT JOIN user_profiles up ON up.user_id = u.id
         WHERE u.is_banned = 0
